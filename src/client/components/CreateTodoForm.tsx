...
export const CreateTodoForm = () => {
  ...
  const newTodo = (event: { key: string }) => {
    if (event.key === 'Enter') {
      createTodo({
        body: todoBody,
      })
    ...
    <>
      <form className="group flex items-center justify-between rounded-12 border border-gray-200 py-2 pr-4 focus-within:border-gray-400">
        ...
        <input
          ...
          onKeyDown={newTodo}
          className="w-72 flex-auto px-4 text-base placeholder:text-gray-400 focus:outline-none"
        />

        <button
          type="button"
          className=" h-9 w-6 flex-auto border-spacing-0 rounded-full bg-gray-950 text-white"
